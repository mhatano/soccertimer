...
* Can select regulation to forty-minutes half or forty-five-minutes half.
* Click "START" to start first half clock. The button caption is changed to "1st HALF STARTED".
* Click "HALF TIME" to stop and reset half clock to 0, total clock to regulation time minutes. Start button caption will be changed to "2nd HALF START", and stop button caption will be "FULL TIME".
* Additional time for the first half is cleared when half time.
* Click "2nd HALF START" to start the second half clock. Half clock is counting from 0, total clock is couting from regulation half time minutes. Caption is changed to "2nd HALF STARTED"
...
This clock shows the clock counting even after the regulation time is passed. Normally stadium clock is off after regulation time passed, so you can use to check the game time with this application.
